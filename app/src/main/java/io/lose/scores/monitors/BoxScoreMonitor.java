...
import io.lose.scores.operations.GetBoxScoreOperation;
...
public class BoxScoreMonitor extends AbstractRequestMonitor {
    ...
    public int onPostExecute(final Context context, final Query request, final QueryResult result) {
        ...
        if (!mSynced) {
            final Operation operation = new GetBoxScoreOperation(request.getWhereArgs()[0]);
